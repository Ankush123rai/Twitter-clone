    ...
    <div className={style.postss} key={user.id}>
      <div className={style.postss__first}>
        ...
        <div className={style.posts__first__username}>
          {user.email}<span>{user.time}</span>
        </div>
        <Tooltip title="More">
        <span className="more">   
          <MoreHorizSharpIcon /> 
      ...
      <div className={style.postss__details}>
        <div className={style.postss__details__msg}>
         {user.content}
        </div>
        <div className={style.postss__details__img}>
          <img onDoubleClick={handleLike} src={`https://picsum.photos/1000/500?q=${user.retweet}`} alt="post" />
        </div>
